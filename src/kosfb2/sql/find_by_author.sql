SELECT 
    ...
    authors.fullname as fullname, -- 3 4 5 6 Авторы  
    genres.gname as gname, -- 7 Жанры
    sequences.sname as sname, sequences.svolume as svolume, -- 8 9 Серии
    publisher.pname as pname, -- 10 Издатель
    pubsequences.psname as psname, pubsequences.psvolume as psvolume, -- 11 12 Издательские серии
    ...
    FROM 
        ...
        INNER JOIN
            (SELECT 
                ...
                array_agg((A.lastname, A.firstname, A.middlename, A.nickname)::authortype) as fullname
            FROM 
                book B
                LEFT JOIN 
                    bookauthor BA
...
SELECT 
    ...
    genres.gname as gname, -- 7 Жанры
    sequences.fullseqname as fullseqname, -- 8 9 Серии
    publisher.pname as pname, -- 10 Издатель
    pubsequences.fullpubseqname as fullpubseqname, -- 11 12 Издательские серии
    books.zipfile as zipfile, -- 13 Файл архива книги
    books.annotation as annotation, -- 14 Описание книги
    books.fb2id as fb2id -- 15 FB2 идентификатор книги
    ...
    FROM 
        ...
        INNER JOIN     
            (SELECT 
                MIN(B.uid) as uid, -- 1 PRIMARY KEY
                array_agg((S.name, BS.volume)::fullseqname) as fullseqname -- 8 9 Серии 
            FROM 
                book B
                LEFT JOIN 
                    booksequence BS
        ...
        INNER JOIN     
            (SELECT 
                MIN(B.uid) as uid, -- 1 PRIMARY KEY
                array_agg((PS.name, BPS.volume)::fullseqname) as fullpubseqname -- 11 12 Издательские серии
            FROM 
                book B
                LEFT JOIN 
                    bookpubsequence BPS
...
-- ORDER BY gname
-- ORDER BY fullseqname
-- ORDER BY fullpubseqname
